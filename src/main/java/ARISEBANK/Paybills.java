...
public class Paybills extends JPanel {
    public Paybills(JFrame frame) {
        setLayout(null);

        JLabel backgroundLabel = new JLabel(new ImageIcon("src/main/resources/Photos/pay.png"));
        ...
        JLabel transferLabel = new JLabel("PAYBILLS");
        transferLabel.setFont(new Font("Arial", Font.BOLD, 20));
        transferLabel.setBounds(245, 10, 300, 30);
        backgroundLabel.add(transferLabel);

         JButton electricbillButton = createButton("INTERNET", 415, 110, 150, _ -> {
           
            frame.setContentPane(new PayInternet(frame));
            frame.validate();
        });
        backgroundLabel.add(electricbillButton);

        JButton internetbillButton = createButton("LOAN ", 415, 250, 150, _ -> {
           
            frame.setContentPane(new PayLoan(frame));
            frame.validate();
        });
        backgroundLabel.add(internetbillButton);

        JButton electricButton = createButton("ELECTRICITY", 120, 110, 150, _ -> {
            frame.setContentPane(new PayElectric(frame));
            frame.validate();
            frame.repaint();
        });
        backgroundLabel.add(electricButton);
        
        JButton savingsAccount = createButton("WATER ", 120, 250, 150, _ -> {
           
            frame.setContentPane(new PayWater(frame));
            frame.validate();
        });
